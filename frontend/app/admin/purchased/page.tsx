...
export default function AdminPurchasedPage() {
    ...
    const tableItems = useMemo(() => filteredItems.filter(i => i.isPurchased && !i.isDistributed && !i.isUnavailable), [filteredItems]);
    ...
    return (
        <div>
            <Header
                title="구매 현황판"
                description="구매가 완료되어 배부를 대기 중인 품목들을 관리하세요."
            />
            ...
            <div className="bg-white border border-[#E5E7EB] rounded-lg px-4 py-3 mb-4 flex flex-wrap items-center gap-3">
                ...
                {tableItems.length !== items.filter(i => i.isPurchased && !i.isDistributed && !i.isUnavailable).length && (
            ...
            <div className="bg-white border border-[#E5E7EB] rounded-lg overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
                {error ? (
                    ...
                    <div className="py-16 text-center">
                        ...
                        <p className="text-sm text-[#64748B]">해당 기간에 구매된 항목이 없습니다.</p>
                    </div>
                ) : (
                    <>
                        {/* 데스크톱 테이블 뷰 */}
